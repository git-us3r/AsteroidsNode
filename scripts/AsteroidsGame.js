...
EXPD.Sounds = {
	...
	explosion : 'sounds/gameSounds/explosion',
	gamePlay : 'sounds/gameSounds/gamePlay',
	lazer : 'sounds/gameSounds/lazer',
	teleport : 'sounds/gameSounds/teleport'
};



EXPD.playSound = function(_sound) {

	var tempSound = {};	
	tempSound = EXPD.sounds[_sound + '.' + EXPD.audioExt].cloneNode();
	tempSound.play();
...
EXPD.makeShipSafe = function(){
	...
	// use the warp function to reset..... TODO
...
EXPD.initialize = function(){
	...
	// initialize scores
	// INTERFACE: create(_CurrentScore, _LivesRemaining, _LivesMax)
	///
	EXPD.Scores.Create(0, 3, 3);
...
EXPD.fireFriendlyMissile = function(_elapsedTime, _code){
	...
	if(_code !== -1){
		...
		// Insert.
		EXPD.playSound(EXPD.Sounds.lazer);
...
EXPD.collisionDetection = function(){
	...
	function collisionStrategy_exp(ntt1, ntt2){
		...
		switch(tempAst.type){
			...
			case  EXPD.Entities.ASTEROID_LRG:
				EXPD.Scores.UpdateScore(20); 		//TODO: set up vars
				break;
				
			case  EXPD.Entities.ASTEROID_MED:
				EXPD.Scores.UpdateScore(50); 		//TODO: set up vars
				break;
				
			case  EXPD.Entities.ASTEROID_SML:
				EXPD.Scores.UpdateScore(100); 		//TODO: set up vars
				break;
		...
		EXPD.playSound(EXPD.Sounds.explosion);
	...
	function collisionStrategy_shipCrash(ntt1, ntt2) {
		...
		if(EXPD.Scores.CurrentLife() === 0){

			EXPD.gameOver();
		}
		else {

			EXPD.Scores.UpdateLife(-1);
			...
			// This should be the actual warping behavior
			EXPD.makeShipSafe();
		}
...
EXPD.update = function(elapsedTime, canvasDim){
	...
	(function preCollisionUpdate(){
		...
		for(var ast in EXPD.asteroids){
			if(EXPD.asteroids.hasOwnProperty(ast)){
		...
		for(var msl in EXPD.friendlyMissles){
			if(EXPD.friendlyMissles.hasOwnProperty(msl)){
	...
	(function postCollisionUpdate(){
		...
		for(var exp in EXPD.explosions){
			...
			if(EXPD.explosions.hasOwnProperty(exp)){
...
EXPD.render = function(ctx){
	...
	for (var flash in EXPD.flashes){
		...
		// A better location for this is in the flash function (in case there are several)
		// TODO : relocate.
...
EXPD.screens['game-play'] = (function() {
	...
	function run() {
		...
		EXPD.playSound(EXPD.Sounds.gamePlay);
